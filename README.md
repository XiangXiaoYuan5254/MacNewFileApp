...
- 创建后自动在 Finder 中选中新文件；授予辅助功能权限后，会自动进入重命名状态
- 支持右键 **拷贝路径**，选中文件复制文件路径，选中文件夹复制文件夹路径
- 支持右键 **打开终端**，自动进入当前目录
- 右键创建时后台执行，不弹出主窗口
...
6. 如果系统设置打开了辅助功能页面，请允许 **NewFileApp**
7. 在 Finder 文件夹空白处右键，选择 **新建文件**

macOS 不允许第三方 App 完全静默启用 Finder 扩展，所以“系统设置里启用扩展”这一步可能需要手动点一次。
自动进入重命名需要辅助功能权限；如果没有授予权限，新文件仍会被选中，可以手动按回车重命名。
